...
const Quiz = () => {
  ...
  const { category, difficulty, totalQuestion, type } = useQuizStore();
    ...
    const getQuizData = async () => {
      try {
        const response = await API.get(
          `/quiz?amount=${totalQuestion}&category=${category}&difficulty=${difficulty}&type=${type}`,
          {}
        );
    ...
    <div className="h-screen">
      <div className="border-cyan-50 shadow-md  shadow-green-500 border-2 bg-gradient-to-r mt-6 max-w-sm px-5 from-cyan-300 to-pink-400 md:max-w-3xl m-auto rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 mt-2 gap-y-5 gap-x-4 content-center max-h-max md:h-60 ">
          {currentQuestion.answers.map((answer, index) => (
            <button
              ...
              onClick={() =>
                handleAnswer(answer === currentQuestion.correct_answer, index)
              }
            >
              {answer}
            </button>
        ...
        <div className=" mb-2 mt-9 text-center">
          {timer < 4 ? (
            <h3 className="text-4xl mt-4 font-bold text-red-600 ">{timer}</h3>
          ) : (
            <h3 className="text-5xl font-bold dark:text-white ">{timer}</h3>
          )}
        </div>
      </div>
    </div>
